...
if [ $mode = "new" ] || [ $mode = "reconfig" ]; then
   ...
   echo "echo \"select seq_no || '-' || hex(ledger_hash) from ledger order by seq_no DESC limit 1;\" | sqlite3 $contdir/ledger_fs/seed/primary/\$max_shard_no/ledger.sqlite" >> $contdir/lcl.sh
