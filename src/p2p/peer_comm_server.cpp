...
namespace p2p
{
    ...
    void peer_comm_server::maintain_known_connections()
    {
        ...
        for (const auto &peer : peer_check_list)
        {
            ...
            std::variant<hpws::client, hpws::error> client_result = hpws::client::connect(conf::ctx.hpws_exe_path, max_msg_size, host, port, "/", conf::cfg.contract.id, {}, util::fork_detach);
